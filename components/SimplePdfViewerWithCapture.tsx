...
interface SimplePdfViewerWithCaptureProps {
  url: string
  onPageChange?: (page: number, totalPages: number) => void
  onCanvasReady?: (getImage: () => string | null) => void
}

export default function SimplePdfViewerWithCapture({ 
  url, 
  onPageChange,
  onCanvasReady 
}: SimplePdfViewerWithCaptureProps) {
  ...
  // Provide the canvas capture function
  useEffect(() => {
    if (onCanvasReady) {
      onCanvasReady(() => {
  ...
  const handlePageRender = () => {
    // Get the canvas element rendered by react-pdf
