...
interface InteractiveLessonPageViewerProps {
  lessonId: string
  documentId: string
...
export default function InteractiveLessonPageViewer({ 
  lessonId, 
  documentId, 
  onPageChange, 
  onCanvasReady 
}: InteractiveLessonPageViewerProps) {
  const [url, setUrl] = useState<string | null>(null)
  const [error, setError] = useState(false)
  const canvasRef = useRef<HTMLCanvasElement | null>(null)

  useEffect(() => {
    fetch(`/api/interactive-lessons/${lessonId}/documents/${documentId}/signed-url`, { credentials: 'include' })
      .then(r => r.json())
