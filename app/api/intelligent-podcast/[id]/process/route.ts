  ...
  try {
    ...
    if (!hasScript) {
      ...
      try {
        ...
        const { data: publicData } = supabaseAdmin.storage.from('podcast-documents').getPublicUrl(transcriptPath)
        await supabase
          .from('intelligent_podcasts')
    ...
    for (let i = 0; i < batchWithAudio.length; i++) {
      ...
      const { data: publicData } = getSupabaseAdmin().storage.from('podcast-audio').getPublicUrl(path)
