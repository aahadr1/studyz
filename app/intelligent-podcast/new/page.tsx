...
export default function NewPodcastPage() {
  ...
  const uploadViaApi = async (file: File | Blob, bucket: string, path: string, contentType: string): Promise<string> => {
    ...
    const res = await fetch('/api/intelligent-podcast/upload', { method: 'POST', body: formData })
    if (!res.ok) {
  ...
  const prepareFilesInBackground = async (filesToPrepare: UploadedFile[]) => {
    ...
    for (const fileObj of filesToPrepare) {
      try {
        ...
        await uploadViaApi(fileObj.file, 'podcast-documents', pdfStoragePath, 'application/pdf')
        ...
        for (const p of pageImages) {
          ...
          const publicUrl = await uploadViaApi(blob, 'podcast-documents', pagePath, 'image/jpeg')
          uploaded.push({ page_number: p.pageNumber, url: publicUrl })
