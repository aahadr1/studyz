...
import { FiLogOut, FiHome, FiPlus, FiArrowRight, FiCheckSquare, FiZap, FiMic, FiLayers } from 'react-icons/fi'
...
export default function DashboardPage() {
  ...
  const [flashcardDecksCount, setFlashcardDecksCount] = useState(0)
    ...
    const loadDashboard = async () => {
      ...
      if (session) {
        ...
        if (flashcardsRes.ok) {
          ...
          setFlashcardDecksCount((data.decks || []).length)
        }
      }
      } catch (err: any) {
    ...
    <div className="min-h-screen bg-background flex">
      ...
      <aside className="w-60 sidebar flex flex-col">
        ...
        <nav className="flex-1 py-4">
          ...
          <Link href="/flashcards" className="sidebar-item">
      ...
      <main className="flex-1 overflow-auto">
        ...
        <div className="p-8 max-w-5xl">
          ...
          <div className="grid grid-cols-4 gap-4 mb-10">
            ...
            <div className="p-6 bg-elevated border border-border rounded-xl">
              <span className="block text-3xl font-semibold mono text-text-primary">{flashcardDecksCount}</span>
              <span className="text-xs text-text-secondary uppercase tracking-wider">Flashcard Decks</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-10">
            ...
            <Link href="/flashcards/new" className="bg-elevated border border-border rounded-xl p-6 group hover:bg-hover hover:border-border-light hover:shadow-md transition-all">
              <div className="flex items-start justify-between">
                <div>
                  <div className="w-12 h-12 border border-border rounded-xl flex items-center justify-center mb-4 group-hover:border-text-primary group-hover:bg-surface transition-all">
                    <FiLayers className="w-5 h-5" strokeWidth={1.5} />
                  </div>
                  <h3 className="text-lg font-medium text-text-primary mb-1">New Flashcards</h3>
                  <p className="text-sm text-text-secondary">AI-powered spaced repetition</p>
                </div>
                <FiArrowRight className="w-4 h-4 text-text-tertiary group-hover:text-text-primary transition-colors" strokeWidth={1.5} />
              </div>
            </Link>
