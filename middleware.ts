...
const desktopToMobileRoutes: Record<string, string> = {
  ...
  '/flashcards/new': '/m/flashcards/new',
}
...
const mobileToDesktopRoutes: Record<string, string> = {
  ...
  '/m/flashcards/new': '/flashcards/new',
}
...
export async function middleware(req: NextRequest) {
  ...
  const mobileFlashcardMatch = pathname.match(/^\/m\/flashcards\/([^\/]+)$/)
  ...
  if (isMobile && !pathname.startsWith('/m')) {
    ...
    // Check dynamic MCQ routes
      ...
      mobileUrl = `/m/flashcards/${flashcardMatch[1]}`
    }
  ...
  if (!isMobile && pathname.startsWith('/m')) {
    ...
    else if (mobileFlashcardMatch) {
      desktopUrl = `/flashcards/${mobileFlashcardMatch[1]}`
    }
